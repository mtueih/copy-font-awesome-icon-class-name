...
// （「图标类名复制按钮」将会插入到其后。）
const URL_CPY_BTN_SELECTORS = ['button[aria-label="Copy Icon URL"]'];
// 包含「图标类名」的元素的选择器。
...
const CPY_BTN_TOOLTIP_TEXT_COPY_SUCCESS = browser.i18n.getMessage(
  "cpyBtnTooltipTextCopySuccess",
);
const CPY_BTN_TOOLTIP_TEXT_COPY_FAIL = browser.i18n.getMessage(
  "cpyBtnTooltipTextCopyFail",
);
...
function cpyBtnChangeStatus(isSuccess) {
  ...
  // 添加复制（成功/失败）状态类。
  cpyBtn.classList.add(
    isSuccess ? UNIQUE_CPY_BTN_SUCCESS_CLASS : UNIQUE_CPY_BTN_FAIL_CLASS,
  );
...
function copyIconClassName() {
  // 查询包含类名的元素。
  const iconClassNameEl = document.querySelector(
    ICON_CLASS_NAME_EL_SELECTORS.join(","),
  );
  if (!iconClassNameEl) {
  ...
  // 提取类名。
  const iconClassNameMatches = iconClassNameEl.innerText.match(
    ICON_CLASS_NAME_REGEX,
  );
  ...
  // 写入剪切板。
  navigator.clipboard
    .writeText(iconClassNameMatches[ICON_CLASS_NAME_REGEX_GROUP])
    .then(() => {
...
function setCopyButton(cpyBtn) {
  ...
  const btnSvgPath = document.createElementNS(
...
browser.runtime.onMessage.addListener((message) => {
  ...
  waitForElement(URL_CPY_BTN_SELECTORS.join(",")).then((urlCpyBtn) => {
